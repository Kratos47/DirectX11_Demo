...
void ShaderTextureLight::SetDirectionalLightParameters(const Vect dir[2], const Vect& amb, const Vect& dif, const Vect& sp)
{
	DirLightData.Light.Ambient = amb;
	DirLightData.Light.Diffuse = dif;
	DirLightData.Light.Specular = sp;
	DirLightData.Direction[0] = dir[0];
	DirLightData.Direction[1] = dir[1];
}

void ShaderTextureLight::SetPointLightParameters(const Vect pos[2], float r, const Vect& att, const Vect& amb, const Vect& dif, const Vect& sp)
{
	PointLightData.Light.Ambient = amb;
	PointLightData.Light.Diffuse = dif;
	PointLightData.Light.Specular = sp;
	PointLightData.Position[0] = pos[0];
	PointLightData.Position[1] = pos[1];
	PointLightData.Attenuation = att;
	PointLightData.Range = r;
}

void ShaderTextureLight::SetSpotLightParameters(const Vect pos[4], float r, const Vect& att, const Vect& dir, float spotExp, const Vect& amb, const Vect& dif, const Vect& sp)
{
	SpotLightData.Light.Ambient = amb;
	SpotLightData.Light.Diffuse = dif;
	SpotLightData.Light.Specular = sp;

	SpotLightData.Position[0] = pos[0];
	SpotLightData.Position[1] = pos[1];
	SpotLightData.Position[2] = pos[2];
	SpotLightData.Position[3] = pos[3];
...
void ShaderTextureLight::SendFogProperties(const float & FogStart, const float & FogRange, const Vect & FogColor)
{
	Fog fog;
	fog.FogStart = FogStart;
	fog.FogRange = FogRange;
	fog.FogColor = FogColor;
	this->GetContext()->UpdateSubresource(mpBufferFog, 0, nullptr, &fog, 0, 0);
}
...
void ShaderTextureLight::SetTextureResourceAndSampler(Texture *& tex, LPCWSTR filepath, D3D11_FILTER filter, UINT isotropic_level,
...
void ShaderTextureLight::SetToContext(Texture *& tex)
{
	tex->SetToContext(GetContext(), 0, 0);
}
